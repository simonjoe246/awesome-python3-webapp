...
from apis import Page, APIValueError, APIError, APIResourceNotFound, APIPermissionError
...
def user2cookie(user, max_age):
    """
    Generate cookie str by: id-expires-sha1
    """
...
async def cookie2user(cookie_str):
    ...
    :param cookie_str:
    :return: None when cookie is wrong or time is out of max_age
            user when cookie and hash are right
    """
    if not cookie_str:
        return None
    try:
        ...
        # varify whether the cookies is right
...
async def get_blog(id):
    ...
    # 向每条comment对象的html_content属性添加html格式化之后的内容
    for c in comments:
...
def signout(request):
    ...
    r.set_cookie(COOKIE_NAME, '-delated-', max_age=0, httponly=True)
...
async def api_register_users(*, email, name, passwd):
    """
    用户注册，之后根据用户一系列数据向浏览器发送响应：注册结果及cookies
    :return: cookies响应
    """
